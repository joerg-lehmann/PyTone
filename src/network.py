    ...
    def run(self):
        ...
        while not self.done:
            ...
            if type == _EVENT:
                ...
                log.debug("server: requesting %s for client" % repr(obj))
                # extract id
                rid, obj = obj
                result = hub.request(obj, priority=-50)
                log.debug("server: got answer %s" % repr(result))
                ...
                log.debug("server: client requests subscription for '%s'" % repr(obj))
